...
distribute `Letterhead`, or make a modified version available to users
...
case — for example, shipping `Letterhead` as part of a **proprietary
...
`Letterhead` is authored by a single copyright holder (Johannes Kaindl). Because all copyright is
...
license simultaneously, without anyone else's code blocking that option.
