...
Repository: [github.com/johannes-kaindl/obsidian-letterhead](https://github.com/johannes-kaindl/obsidian-letterhead) (kanonisch: [git.jkaindl.de/jkaindl/obsidian-letterhead](https://git.jkaindl.de/jkaindl/obsidian-letterhead))

### Mit AnySource Sideloader (empfohlen)

[AnySource Sideloader](https://github.com/johannes-kaindl/anysource-sideloader) installiert und
...
Sobald Letterhead im Community-Plugin-Verzeichnis gelistet ist:
...
[letzten Release](https://github.com/johannes-kaindl/obsidian-letterhead/releases/latest)
...
- [Dokumentations-Index](https://github.com/johannes-kaindl/obsidian-letterhead/blob/main/docs/README.md) — alle Anleitungen an einem Ort (englisch).
- [Getting started](https://github.com/johannes-kaindl/obsidian-letterhead/blob/main/docs/tutorial.de.md) — dein erster Brief, von der leeren Notiz zum PDF.
- [Troubleshooting](https://github.com/johannes-kaindl/obsidian-letterhead/blob/main/docs/troubleshooting.md) — die genaue Meldung, ihre Ursache und die Abhilfe (englisch).
...
Letterhead läuft vollständig auf deinem Gerät: keine Netzwerkaufrufe, keine Telemetrie, kein Tracking. Die Quelle ist TypeScript in `src/`, lesbar und prüfbar; `main.js` selbst ist Build-Output, keine committete Datei. Der einzige `btoa()`-Aufruf bettet dein konfiguriertes Logo als inline `data:`-URL ein. Über den Release-Workflow gebaute GitHub-Releases tragen eine Sigstore/SLSA-Build-Provenance-Attestation, die GitHub Actions frisch aus der getaggten Quelle erzeugt — prüfe mit `gh attestation verify main.js --repo johannes-kaindl/obsidian-letterhead`, dass das laufende `main.js` aus dieser Quelle stammt. 1.6.5 und 1.6.6 wurden nicht so gebaut und haben keine Attestation; sie liefern stattdessen `checksums.sha256`; [`SECURITY.de.md`](https://github.com/johannes-kaindl/obsidian-letterhead/blob/main/SECURITY.de.md) erklärt, was welcher Weg belegt, wie du einen Build in der Zwischenzeit selbst prüfst und wie du Sicherheitslücken meldest.
